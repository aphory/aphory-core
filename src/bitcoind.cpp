 ...
 *
 * This is the developer documentation of the reference client for an experimental new digital currency called Aphory (https://www.aphory.io/),
 * which enables instant payments to anyone, anywhere in the world. Aphory uses peer-to-peer technology to operate
...
static bool AppInit(int argc, char* argv[])
{
    ...
    if (HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        {
            strUsage += "\nUsage:  aphoryd [options]                     Start " PACKAGE_NAME " Daemon\n";
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                fprintf(stderr, "Error: Command line contains unexpected token '%s', see aphoryd -h for a list of options.\n", argv[i]);
                exit(EXIT_FAILURE);
            }
        }
...
#endif
            fprintf(stdout, "Aphory server starting\n");

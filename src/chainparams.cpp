...
{
    static const int64_t nSecondsInHalfYear = 365 * 24 * 60 * 60 / 2;
    static const int64_t nStartPercent = 8;

    if (strNetworkID != "regtest") {
        int64_t nHalfYearsSinceGenesis = (nTime - genesis.nTime) / nSecondsInHalfYear;

        if (nHalfYearsSinceGenesis >= 0) {
            return std::max<int64_t>(nCoinYearReward, (nStartPercent - nHalfYearsSinceGenesis) * CENT);
        }
    }
...
{
    const char* pszTimestamp = "June 2019, Free Markets for the World.";
    const CScript genesisOutputScript = CScript() << ParseHex("0452a9f3d821c7ca80372c6665d296a5844bdcb6a928146179e79b96a2d9bde3577e4b7319fafe235a2839a7b23bcb320ba8d8ad36bd62c0d29b83eb1829121750") << OP_CHECKSIG;
...
const std::pair<const char*, CAmount> regTestOutputs[] = {
    std::make_pair("01fcca2c001570606591bcd1a32213b94b36a164", 50000 * COIN),
    std::make_pair("01fcca2c001570606591bcd1a32213b94b36a164", 50000 * COIN),
    std::make_pair("01fcca2c001570606591bcd1a32213b94b36a164", 50000 * COIN),
    std::make_pair("01fcca2c001570606591bcd1a32213b94b36a164", 50000 * COIN),
    std::make_pair("01fcca2c001570606591bcd1a32213b94b36a164", 50000 * COIN),

    std::make_pair("01fcca2c001570606591bcd1a32213b94b36a164", 10000 * COIN),
    std::make_pair("01fcca2c001570606591bcd1a32213b94b36a164", 10000 * COIN),
    std::make_pair("01fcca2c001570606591bcd1a32213b94b36a164", 10000 * COIN),
    std::make_pair("01fcca2c001570606591bcd1a32213b94b36a164", 10000 * COIN),
    std::make_pair("01fcca2c001570606591bcd1a32213b94b36a164", 10000 * COIN),
...
const std::pair<const char*, CAmount> genesisOutputs[] = {
    std::make_pair("235682f65411f30809efeea19ef3e41095fea180", 1000 * COIN),
    std::make_pair("235682f65411f30809efeea19ef3e41095fea180", 1000 * COIN),
    std::make_pair("235682f65411f30809efeea19ef3e41095fea180", 1000 * COIN),
    std::make_pair("235682f65411f30809efeea19ef3e41095fea180", 1000 * COIN),
    std::make_pair("235682f65411f30809efeea19ef3e41095fea180", 1000 * COIN),
...
const std::pair<const char*, CAmount> genesisOutputsTestnet[] = {
    std::make_pair("cc37af701b4b072f1c194793fab05ed78dde5a49", 50000 * COIN),
    std::make_pair("cc37af701b4b072f1c194793fab05ed78dde5a49", 50000 * COIN),
    std::make_pair("cc37af701b4b072f1c194793fab05ed78dde5a49", 20000 * COIN),
    std::make_pair("cc37af701b4b072f1c194793fab05ed78dde5a49", 20000 * COIN),
    std::make_pair("cc37af701b4b072f1c194793fab05ed78dde5a49", 20000 * COIN),
    std::make_pair("cc37af701b4b072f1c194793fab05ed78dde5a49", 10000 * COIN),
    std::make_pair("cc37af701b4b072f1c194793fab05ed78dde5a49", 10000 * COIN),
    std::make_pair("cc37af701b4b072f1c194793fab05ed78dde5a49", 10000 * COIN),
    std::make_pair("cc37af701b4b072f1c194793fab05ed78dde5a49", 10000 * COIN),
    std::make_pair("cc37af701b4b072f1c194793fab05ed78dde5a49", 10000 * COIN),
...
public:
    CMainParams() {
        ...
        consensus.nSubsidyHalvingInterval = 210000 * 2;
        ...
        consensus.nMinimumChainWork = uint256S("0000000000000000000000000000000000000000000000000d8922ed57d46dfc");

        // By default assume that the signatures in ancestors of this block are valid.
        consensus.defaultAssumeValid = uint256S("0x000012dbb872ab2abec87f5eff5ee0f54e7a5d7de5290633429d4adc8e57defb"); // genesis
        ...
        nStakeMinConfirmations = 500;   // 500 * 1 minutes
        nTargetSpacing = 1 * 60;        // 1 minute
        nTargetTimespan = 12 * 60;      // 12 mins
        ...
        genesis = CreateGenesisBlockMainNet(1559822710, 74007, 0x1f00ffff);
        // FindGenesisBlock(genesis);
        consensus.hashGenesisBlock = genesis.GetHash();

        assert(consensus.hashGenesisBlock == uint256S("0x000012dbb872ab2abec87f5eff5ee0f54e7a5d7de5290633429d4adc8e57defb"));
        assert(genesis.hashMerkleRoot == uint256S("0x51b42033515c15f507002e2701011d21fc2d86bbc4ad982875a406bb0cb2ab17"));
        assert(genesis.hashWitnessMerkleRoot == uint256S("0xa52cb554b0ba7a0f3da0c06439b88c7ab68b933ebff3d373ffb9ec68675bc6e1"));
        ...
        vSeeds.emplace_back("dnsseed.aphory.io");
        vSeeds.emplace_back("dnsseed2.aphory.io");

        vDevFundSettings.emplace_back(0,
            DevFundSettings("AJziw9B43SqPrRj3XQAi1xPgdh2LmPGfBg", 10, 60));


        base58Prefixes[PUBKEY_ADDRESS]     = {0x17}; // A
        base58Prefixes[SCRIPT_ADDRESS]     = {0x3c};
        base58Prefixes[PUBKEY_ADDRESS_256] = {0x18};
        ...
        base58Prefixes[EXT_ACC_HASH]       = {0x2e}; // K
        base58Prefixes[EXT_PUBLIC_KEY_BTC] = {0x04, 0x88, 0xB2, 0x1E}; // xpub
        base58Prefixes[EXT_SECRET_KEY_BTC] = {0x04, 0x88, 0xAD, 0xE4}; // xprv

        bech32Prefixes[PUBKEY_ADDRESS].assign       ("ah","ah"+2);
        bech32Prefixes[SCRIPT_ADDRESS].assign       ("ar","ar"+2);
        bech32Prefixes[PUBKEY_ADDRESS_256].assign   ("al","al"+2);
        bech32Prefixes[SCRIPT_ADDRESS_256].assign   ("aj","aj"+2);
        bech32Prefixes[SECRET_KEY].assign           ("ax","ax"+2);
        bech32Prefixes[EXT_PUBLIC_KEY].assign       ("aep","aep"+3);
        bech32Prefixes[EXT_SECRET_KEY].assign       ("aex","aex"+3);
        bech32Prefixes[STEALTH_ADDRESS].assign      ("as","as"+2);
        bech32Prefixes[EXT_KEY_HASH].assign         ("aek","aek"+3);
        bech32Prefixes[EXT_ACC_HASH].assign         ("aea","aea"+3);
        bech32Prefixes[STAKE_ONLY_PKADDR].assign    ("acs","acs"+3);
        ...
        checkpointData = {
            {
                { 0,     uint256S("0x000012dbb872ab2abec87f5eff5ee0f54e7a5d7de5290633429d4adc8e57defb")},
                { 200,   uint256S("0xd310969c42aefcad0cc255774d36369b6d8d10d3c1c7842b21b2a9b636aa5fb6")}, // after airdrops
            }
        };

        chainTxData = ChainTxData {
            // Data from rpc: getchaintxstats 180 d310969c42aefcad0cc255774d36369b6d8d10d3c1c7842b21b2a9b636aa5fb6
            /* nTime    */ 1559999712,
            /* nTxCount */ 216,
            /* dTxRate  */ 0.016
        };
...
public:
    CTestNetParams() {
        ...
        // By default assume that the signatures in ancestors of this block are valid.
        consensus.defaultAssumeValid = uint256S("0x0000ab513dec84c42f9092b5e061421e3124ccf40808affadf7b3d3487259775");
        ...
        nModifierInterval = 10 * 60;    // 10 minutes
        nStakeMinConfirmations = 210;   // 210 * 1 minutes
        ...
        genesis = CreateGenesisBlockTestNet(1559822710, 23492, 0x1f00ffff);
        // FindGenesisBlock(genesis);
        consensus.hashGenesisBlock = genesis.GetHash();

        assert(consensus.hashGenesisBlock == uint256S("0x0000ab513dec84c42f9092b5e061421e3124ccf40808affadf7b3d3487259775"));
        assert(genesis.hashMerkleRoot == uint256S("0x978ef1268a79a3a98e87f0a080bdcf54a10a739e7a2c6c411e87de89577bea6b"));
        assert(genesis.hashWitnessMerkleRoot == uint256S("0x8f226cf2c977b91c2667e52238a2561e0269e9a92b6cb2031bae98d8f651f438"));

        vFixedSeeds.clear();
        vSeeds.clear();
        // nodes with support for servicebits filtering should be at the top
        vSeeds.emplace_back("testdnsseed.aphory.io");
        vSeeds.emplace_back("testdnsseed2.aphory.io");

        vDevFundSettings.push_back(std::make_pair(0, DevFundSettings("pjK9gkG1R1V3o8XtGw3igh588UiA5Rgde7", 10, 60)));
        ...
        checkpointData = {
            {
                {0, uint256S("0x0000ab513dec84c42f9092b5e061421e3124ccf40808affadf7b3d3487259775")},
            }
        };
...
public:
    explicit CRegTestParams(const ArgsManager& args) {
        ...
        genesis = CreateGenesisBlockRegTest(1559822712, 1, 0x207fffff);
        // FindGenesisBlock(genesis);
        consensus.hashGenesisBlock = genesis.GetHash();
        assert(consensus.hashGenesisBlock == uint256S("0x2ac419a5f5dff39e9cd4549049e8a7186e316ad14d991f7c8625b98a5442a491"));
        assert(genesis.hashMerkleRoot == uint256S("0x3f463b74799d6ddbfc6e45bacba4cfe40ec27cd5c33c03ed2604507aefd94834"));
        assert(genesis.hashWitnessMerkleRoot == uint256S("0x7bf178391212ca5bb22c6f38fa9631e870d93c6b1a4819ccc922bab127acc8c8"));
        ...
        checkpointData = {
            {
                {0, uint256S("0x2ac419a5f5dff39e9cd4549049e8a7186e316ad14d991f7c8625b98a5442a491")},
            }
        };

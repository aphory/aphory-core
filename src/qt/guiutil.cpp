...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent, bool allow_stakeonly)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Aphory address (e.g. %1)").arg(
...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(!uri.isValid() || uri.scheme() != QString("aphory"))
        return false;
...
{
    QString ret = QString("aphory:%1").arg(info.address);
    int paramCount = 0;
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Aphory.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Aphory (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Aphory (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "aphory.desktop";
    return GetAutostartDir() / strprintf("aphory-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Aphory\n";
        else
            optionFile << strprintf("Name=Aphory (%s)\n", chain);

...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " aphory-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  aphory-tx [options] <hex-tx> [commands]  Update hex-encoded bitcoin transaction\n" +
            "or:     aphory-tx [options] -create [commands]   Create hex-encoded bitcoin transaction\n" +
            "\n";
...
{
    int64_t newVersion;
    if (!ParseInt64(cmdVal, &newVersion) || newVersion < 1 || newVersion > CTransaction::MAX_STANDARD_APHORY_VERSION)
        throw std::runtime_error("Invalid TX version requested: '" + cmdVal + "'");

    if (!tx.IsAphoryVersion() && IsAphoryTxVersion(newVersion)) {
...
static void MutateTxSign(CMutableTransaction& tx, const std::string& flagStr)
{
    ...
    for (unsigned int i = 0; i < mergedTx.vin.size(); i++) {
        ...
        if (tx.IsAphoryVersion() && amount == 0)
...
{
    if (!tx.IsAphoryVersion())
        throw std::runtime_error("tx not aphory version.");
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        tx.nVersion = CTransaction::CURRENT_APHORY_VERSION;
        int startArg;

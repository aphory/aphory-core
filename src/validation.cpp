...
#if defined(NDEBUG)
# error "Aphory cannot be compiled without assertions."
#endif
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (!fAphoryMode  // genesis coinbase is spendable when in Aphory mode
        && fIsGenesisBlock) {
        if (!fJustCheck)
    ...
    if (fAphoryMode || VersionBitsState(pindex->pprev, consensus, Consensus::DEPLOYMENT_CSV, versionbitscache) == ThresholdState::ACTIVE) {
    ...
    blockundo.vtxundo.reserve(block.vtx.size() - (fAphoryMode ? 0 : 1));
    ...
    if (fAphoryMode) {
        ...
        if (block.vtx[0]->GetValueOut() > blockReward) // Aphory coins are imported as coinbase txns
...
void UpdateTip(const CBlockIndex *pindexNew, const CChainParams& chainParams) {
    ...
    if (!IsInitialBlockDownload())
    {
        ...
        for (int i = 0; i < 100 && pindex != nullptr; i++)
        {
            if (fAphoryMode)
            {
                if (pindex->nVersion > APHORY_BLOCK_VERSION)
                    ++nUpgraded;
            } else
            {
...
bool CChainState::AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        if (miSelf != mapBlockIndex.end()) {
            ...
            if (fAphoryMode && !IsInitialBlockDownload() && state.nodeId >= 0
...
bool CChainState::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const CDiskBlockPos* dbp, bool* fNewBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (!pindex->pprev
            ...
            if (fAphoryMode && !IsInitialBlockDownload()) {

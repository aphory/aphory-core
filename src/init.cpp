...
{
    // Create a message-only window to intercept WM_CLOSE events from aphoryd
 ...
 */
static const char* BITCOIN_PID_FILENAME = "aphory.pid";
...
{
#ifdef WIN32
    // Only aphoryd will create a hidden window to receive messages
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-rpccorsdomain=<domain>", "Allow JSON-RPC connections from specified domain (e.g. http://localhost:4200 or \"*\"). This needs to be set if you are using the Aphory GUI in a browser.", false, OptionsCategory::RPC);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/aphory/aphory>";
    const std::string URL_WEBSITE = "<https://aphory.io/>";
...
{
    fAphoryMode = !gArgs.GetBoolArg("-btcmode", false); // qa tests
    if (!fAphoryMode) {
        WITNESS_SCALE_FACTOR = WITNESS_SCALE_FACTOR_BTC;
        if (gArgs.GetBoolArg("-regtest", false)) {
            ResetParams(CBaseChainParams::REGTEST, fAphoryMode);
        }
    }
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    if (fAphoryMode && gArgs.GetBoolArg("-smsg", true)) { // SMSG breaks functional tests with services flag, see version msg
#ifdef ENABLE_WALLET

...
static const uint8_t APHORY_BLOCK_VERSION = 0xA0;
static const uint8_t APHORY_TXN_VERSION = 0xA0;
static const uint8_t MAX_APHORY_TXN_VERSION = 0xBF;
...
inline bool IsAphoryTxVersion(int nVersion)
{
    return (nVersion & 0xFF) >= APHORY_TXN_VERSION;
}
...
inline void SerializeTransaction(const TxType& tx, Stream& s) {
    ...
    if (IsAphoryTxVersion(tx.nVersion)) {
...
public:
    ...
    static const int32_t CURRENT_APHORY_VERSION=0xA0;
    ...
    static const int32_t MAX_STANDARD_APHORY_VERSION=0xA0;
...
public:
    ...
    bool IsAphoryVersion() const {
        return IsAphoryTxVersion(nVersion);
    }
    ...
    {
        return IsAphoryTxVersion(nVersion) ? vpout.size() : vout.size();
    }

...
class WalletAphoryTest(AphoryTestFramework):
    ...
    def aphory_wallet_process(self, *args):
        binary = self.config["environment"]["BUILDDIR"] + '/src/aphory-wallet' + self.config["environment"]["EXEEXT"]
        args = ['-datadir={}'.format(self.nodes[0].datadir), '-regtest'] + list(args)
        return subprocess.Popen([binary] + args, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)

    def assert_tool_output(self, output, *args):
        p = self.aphory_wallet_process(*args)
    ...
    def run_test (self):
        ...
        self.log.info('Test aphory-wallet')
        out = textwrap.dedent('''\
            Wallet info
            ===========
            Encrypted: no

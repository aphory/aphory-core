...
from test_framework.test_aphory import (
    AphoryTestFramework,
    isclose,
    getIndexAtProperty,
)
...
class USBDeviceTest(AphoryTestFramework):
    ...
    def run_test(self):
        ...
        # Check that aphory has been built with USB device enabled
        ...
        if not config["components"].getboolean("ENABLE_USBDEVICE"):
            raise SkipTest("aphoryd has not been built with usb device enabled.")
